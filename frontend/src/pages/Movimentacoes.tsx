...
import { createProduto, listProdutos, type Produto, type ProdutoCategoria } from "@/services/produtos";
...
export default function Movimentacoes() {
  ...
  const [novoProdutoForm, setNovoProdutoForm] = useState({
    sku: "", nome: "", categoria: "peca" as ProdutoCategoria | string,
    marca: "", modelo: "", custo: "0", precoVenda: "0", estoqueMinimo: "1",
  });
  ...
  const criarProdutoMutation = useMutation({
    mutationFn: () => createProduto({
      ...
      custo: Number(novoProdutoForm.custo) || 0,
      precoVenda: Number(novoProdutoForm.precoVenda) || 0,
      estoqueAtual: 0,
      estoqueMinimo: Number(novoProdutoForm.estoqueMinimo) || 1,
    }),
    onSuccess: async (produto) => {
      ...
      setNovoProdutoForm({ sku: "", nome: "", categoria: "peca", marca: "", modelo: "", custo: "0", precoVenda: "0", estoqueMinimo: "1" });
      setProdutoOpen(false);
      toast.success(`Produto "${produto.nome}" criado.`);
    },
    onError: (err) => toast.error(err instanceof Error ? err.message : "Erro ao criar produto."),
  });
  ...
  const produtosFiltrados = useMemo(() => {
    ...
    return lista.filter((p) => p.nome.toLowerCase().includes(q) || p.sku.toLowerCase().includes(q)).slice(0, 20);
  }, [produtosQuery.data, produtoSearch]);

  const handleAddItem = () => {
    if (!selectedProduto) return;
    const qtd = Math.max(1, parseInt(qtdInput) || 1);
    const final = parseInt(finalInput) >= 0 ? parseInt(finalInput) : selectedProduto.estoqueAtual;
    setCart((prev) => {
      const idx = prev.findIndex((i) => i.produto.id === selectedProduto.id);
      if (idx >= 0) {
        return prev.map((i, j) =>
          j === idx ? { ...i, quantidade: i.quantidade + qtd, estoqueFinal: final } : i,
        );
      }
      return [...prev, { tempId: `${selectedProduto.id}-${Date.now()}`, produto: selectedProduto, quantidade: qtd, estoqueFinal: final }];
    });
  ...
  const handleSave = async (e: FormEvent) => {
    ...
    setSaving(true);
    try {
      const notaInfo = nfe.numero ? `NF-e: ${nfe.numero}${nfe.serie ? `/S${nfe.serie}` : ""}${nfe.valorAdicional ? ` (R$ ${nfe.valorAdicional})` : ""}` : "";
      const motivoParts = [motivo, descricaoMotivo, perdasAvarias ? "Perdas/Avarias" : "", notaInfo, obs].filter(Boolean);
      const motivoFinal = motivoParts.join(" | ") || undefined;
      const tipoBackend: MovimentacaoEstoqueTipo = tipo === "transferencia" ? "saida" : tipo as MovimentacaoEstoqueTipo;

      for (const item of cart) {
        if (tipoBackend === "ajuste") {
          await createMovimentacaoEstoque({ produtoId: item.produto.id, tipo: "ajuste", estoqueFinal: item.estoqueFinal, motivo: motivoFinal });
        } else {
          await createMovimentacaoEstoque({ produtoId: item.produto.id, tipo: tipoBackend, quantidade: item.quantidade, motivo: motivoFinal });
        }
      }
      await queryClient.invalidateQueries({ queryKey: ["movimentacoes-estoque"] });
      await queryClient.invalidateQueries({ queryKey: ["produtos"] });
      toast.success(`${cart.length} movimentação${cart.length > 1 ? "ões" : ""} salva${cart.length > 1 ? "s" : ""}.`);
      setCart([]); setObs(""); setMotivo(""); setDescricaoMotivo(""); setPerdasAvarias(false); setTipo("entrada"); setNfe({ numero: "", serie: "", dataEmissao: "", valorAdicional: "" }); setMode("list");
    } catch (err) {
  ...
  return (
    <div className="space-y-0">
      ...
      <div className="mb-5 flex items-center justify-between">
        ...
        {mode === "list" ? (
          <Button className="bg-gradient-primary text-primary-foreground shadow-glow" onClick={() => setMode("form")}>
            <Plus className="h-4 w-4" /> Adicionar novo
          </Button>
        ) : (
          <Button variant="outline" onClick={() => { setMode("list"); setCart([]); }}>
            Cancelar
          </Button>
        )}
      </div>
      ...
      {mode === "form" ? (
        <form onSubmit={handleSave}>
          ...
          <div className="border-b border-border pb-6 mb-6">
            <h2 className="mb-4 text-xl font-semibold">Movimentação de Estoque</h2>
            {/* Tipo + Data + Estoque */}
            <div className="flex flex-wrap items-start gap-6">
              <div>
                <p className="mb-2 text-sm text-muted-foreground">Qual é o tipo de movimentação?</p>
                <div className="flex items-center gap-4">
                  {(["entrada", "saida", "transferencia", "ajuste"] as Tipo[]).map((t) => (
                    <label key={t} className="flex cursor-pointer items-center gap-2 text-sm">
                      <input type="radio" name="tipo" value={t} checked={tipo === t}
                        onChange={() => { setTipo(t); setMotivo(""); setDescricaoMotivo(""); setPerdasAvarias(false); }}
              ...
              {(tipo === "saida" || tipo === "transferencia") && (
                <div>
                  <p className="mb-1 text-xs font-medium uppercase tracking-wide text-muted-foreground">Estoque de origem</p>
              ...
              {(tipo === "entrada" || tipo === "transferencia") && (
                <div>
                  <p className="mb-1 text-xs font-medium uppercase tracking-wide text-muted-foreground">
                    {tipo === "transferencia" ? "Estoque de destino" : "Estoque de destino"}
                  </p>
                  <div className="flex h-10 w-44 items-center rounded-md border border-input bg-secondary/30 px-3 text-sm text-muted-foreground">
                    Estoque Padrão
                  </div>
            ...
            {(tipo === "saida" || tipo === "ajuste") && (
              <div className="mt-4 flex flex-wrap items-end gap-4">
                <div className="min-w-[220px]">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      ...
                      {motivosAtivos.map((m) => <option key={m} value={m}>{m}</option>)}
                    </select>
                  </div>
                </div>
                <div className="flex-1 min-w-[200px]">
                  <p className="mb-1 text-xs font-medium uppercase tracking-wide text-muted-foreground">Descrição do motivo</p>
                  <Input
                ...
                {tipo === "saida" && (
                  <div>
                    <p className="mb-1 text-xs font-medium uppercase tracking-wide text-muted-foreground">Perdas e avarias</p>
            ...
            <div className="mt-4">
              <p className="mb-1 text-xs font-medium uppercase tracking-wide text-muted-foreground">Observações</p>
              <Textarea
                rows={2}
                value={obs}
          ...
          <div className="border-b border-border pb-6 mb-6">
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
              <div>
                <p className="mb-1 text-xs font-medium uppercase tracking-wide text-muted-foreground">Nº NF-e/RPS</p>
                <Input
                  value={nfe.numero}
                  onChange={(e) => setNfe((n) => ({ ...n, numero: e.target.value }))}
                  placeholder="Ex.: 001234"
                />
              </div>
              <div>
                <p className="mb-1 text-xs font-medium uppercase tracking-wide text-muted-foreground">Série</p>
                <Input
                  value={nfe.serie}
                  onChange={(e) => setNfe((n) => ({ ...n, serie: e.target.value }))}
                  placeholder="Ex.: 1"
                />
              </div>
              <div>
              ...
              <div>
                <p className="mb-1 text-xs font-medium uppercase tracking-wide text-muted-foreground">Valor adicional</p>
                <Input
                  type="number"
                  min="0"
                  step="0.01"
                  value={nfe.valorAdicional}
                  onChange={(e) => setNfe((n) => ({ ...n, valorAdicional: e.target.value }))}
                  placeholder="R$ 0,00"
                />
              </div>
            </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border bg-secondary/30 text-xs uppercase tracking-wider text-muted-foreground">
                    <th className="px-3 py-2.5 text-left font-medium w-full">Item</th>
                    <th className="px-3 py-2.5 text-center font-medium whitespace-nowrap">
                      {tipo === "ajuste" ? "Estoque final" : "Qtde"}
                    </th>
                    <th className="px-3 py-2.5 text-center font-medium whitespace-nowrap">Valor un.</th>
                    <th className="px-3 py-2.5 text-center font-medium whitespace-nowrap">Atual</th>
                    <th className="px-3 py-2.5 text-center font-medium whitespace-nowrap">Resultado</th>
                ...
                <tbody>
                  ...
                  <tr className="border-b border-border bg-secondary/10">
                        ...
                        <PopoverTrigger asChild>
                          <Button variant="outline" role="combobox" className="w-full justify-between font-normal text-sm h-9">
                            {selectedProduto
                              ...
                              : <span className="text-muted-foreground">Procurar por...</span>}
                            <ChevronsUpDown className="ml-2 h-3.5 w-3.5 shrink-0 opacity-50" />
                          </Button>
                        </PopoverTrigger>
                        <PopoverContent className="w-[380px] p-0" align="start">
                          <Command>
                            <CommandInput placeholder="Nome ou SKU..." value={produtoSearch} onValueChange={setProdutoSearch} />
                            <CommandList>
                              <CommandEmpty>
                              <div className="py-2 text-center">
                                ...
                                <Button
                                  ...
                                  variant="outline"
                                  onClick={() => {
                                    setNovoProdutoForm((f) => ({ ...f, nome: produtoSearch }));
                              ...
                              <CommandGroup>
                                {produtosFiltrados.map((p) => (
                                  <CommandItem key={p.id} value={p.nome} onSelect={() => {
                                    ...
                                    <Check className={cn("mr-2 h-4 w-4", selectedProduto?.id === p.id ? "opacity-100" : "opacity-0")} />
                                    <div>
                                      <p className="font-medium">{p.nome}</p>
                                      <p className="text-xs text-muted-foreground">{p.sku} · Estoque: {p.estoqueAtual}</p>
                                    </div>
                                  </CommandItem>
                                ))}
                              </CommandGroup>
                    ...
                    <td className="px-3 py-2">
                      <Input
                        ...
                        onChange={(e) => tipo === "ajuste" ? setFinalInput(e.target.value) : setQtdInput(e.target.value)}
                    ...
                    <td className="px-3 py-2">
                      <div className="flex h-9 w-24 items-center justify-center rounded-md border border-input bg-secondary/20 text-sm font-mono text-muted-foreground">
                        {selectedProduto ? formatBRL(selectedProduto.custo) : "R$ 0,00"}
                      </div>
                    </td>
                  ...
                  {cart.map((item) => {
                    const resultado =
                      tipo === "entrada" ? item.produto.estoqueAtual + item.quantidade
                      : tipo === "saida" ? item.produto.estoqueAtual - item.quantidade
                      : item.estoqueFinal;
                    const diff = resultado - item.produto.estoqueAtual;
                    return (
                      <tr key={item.tempId} className="border-b border-border/40 hover:bg-secondary/20">
                        ...
                        <td className="px-3 py-3 text-center font-mono font-semibold">
                          {tipo === "ajuste" ? item.estoqueFinal : item.quantidade}
                        </td>
                        ...
                        <td className="px-3 py-3 text-center">
                          <span className={cn(
                            ...
                            {diff > 0 ? "+" : ""}{diff} → {resultado}
                          </span>
                        </td>
                        <td className="px-3 py-3 text-right">
                          <Button variant="ghost" size="icon" type="button" onClick={() => setCart((p) => p.filter((i) => i.tempId !== item.tempId))}>
            ...
            <DialogContent className="max-w-lg">
              ...
              <div className="grid grid-cols-2 gap-3 py-2">
                <FormField id="np-sku" label="SKU / Código">
                  <Input id="np-sku" value={novoProdutoForm.sku}
                    onChange={(e) => setNovoProdutoForm((f) => ({ ...f, sku: e.target.value }))}
                    placeholder="Ex.: BAT-IP13" required />
                </FormField>
                <FormField id="np-categoria" label="Categoria">
                  <Select value={novoProdutoForm.categoria}
                    onValueChange={(v) => setNovoProdutoForm((f) => ({ ...f, categoria: v }))}>
                    <SelectTrigger id="np-categoria"><SelectValue /></SelectTrigger>
                    <SelectContent>
                ...
                <FormField id="np-nome" label="Nome" className="col-span-2">
                  <Input id="np-nome" value={novoProdutoForm.nome}
                    onChange={(e) => setNovoProdutoForm((f) => ({ ...f, nome: e.target.value }))}
                    placeholder="Ex.: Bateria iPhone 13" required />
                </FormField>
                <FormField id="np-marca" label="Marca">
                  <Select value={novoProdutoForm.marca || "__none__"}
                    onValueChange={(v) => setNovoProdutoForm((f) => ({ ...f, marca: v === "__none__" ? "" : v }))}>
                    <SelectTrigger id="np-marca"><SelectValue placeholder="Selecione" /></SelectTrigger>
                    <SelectContent>
                ...
                <FormField id="np-modelo" label="Modelo">
                  <Input id="np-modelo" value={novoProdutoForm.modelo}
                    onChange={(e) => setNovoProdutoForm((f) => ({ ...f, modelo: e.target.value }))}
                    placeholder="Opcional" />
                </FormField>
                <FormField id="np-custo" label="Custo (R$)">
                  <Input id="np-custo" type="number" min="0" step="0.01"
                    value={novoProdutoForm.custo}
                    onChange={(e) => setNovoProdutoForm((f) => ({ ...f, custo: e.target.value }))} />
                </FormField>
                <FormField id="np-venda" label="Preço de venda (R$)">
                  <Input id="np-venda" type="number" min="0" step="0.01"
                    value={novoProdutoForm.precoVenda}
                    onChange={(e) => setNovoProdutoForm((f) => ({ ...f, precoVenda: e.target.value }))} />
                </FormField>
                <FormField id="np-minimo" label="Estoque mínimo">
                  <Input id="np-minimo" type="number" min="0" step="1"
                    value={novoProdutoForm.estoqueMinimo}
                    onChange={(e) => setNovoProdutoForm((f) => ({ ...f, estoqueMinimo: e.target.value }))} />
                </FormField>
              </div>
              <DialogFooter>
                <Button variant="outline" onClick={() => setNovoProdutoOpen(false)}>Cancelar</Button>
                <Button
                  disabled={!novoProdutoForm.sku.trim() || !novoProdutoForm.nome.trim() || criarProdutoMutation.isPending}
          ...
          <div className="mt-8 flex items-center justify-between border-t border-border pt-4">
            <p className="text-sm text-muted-foreground">
              {cart.length} item{cart.length !== 1 ? "s" : ""} adicionado{cart.length !== 1 ? "s" : ""}
            </p>
            <Button
              type="submit"
          ...
          {movQuery.isLoading ? (
            ...
            <Card className="surface-panel">
              <EmptyState
                ...
                actions={<Button onClick={() => setMode("form")}><Plus className="h-4 w-4" /> Nova movimentação</Button>}
              />
            </Card>
          ) : (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border bg-secondary/40 text-xs uppercase tracking-wider text-muted-foreground">
                      ...
                      <th className="px-5 py-3 text-center font-medium">Anterior</th>
                      <th className="px-5 py-3 text-center font-medium">Posterior</th>
                  ...
                  <tbody>
                    {movimentacoes.map((mov) => {
                      ...
                      return (
                        <tr key={mov.id} className="border-b border-border/40 hover:bg-secondary/20">
                          <td className="px-5 py-3 font-mono text-xs text-muted-foreground whitespace-nowrap">
                            {formatDateTime(mov.createdAt)}
                          </td>
                          <td className="px-5 py-3">
                            <div className="flex items-center gap-2">
                              {isEntrada && <ArrowUp className="h-3.5 w-3.5 text-emerald-500" />}
                              {isSaida && <ArrowDown className="h-3.5 w-3.5 text-red-500" />}
                              {!isEntrada && !isSaida && <ArrowUpDown className="h-3.5 w-3.5 text-primary" />}
                              <span className={cn("font-medium text-xs", isEntrada ? "text-emerald-600" : isSaida ? "text-red-500" : "text-primary")}>
                                {tipoLabel[mov.tipo]}
                              </span>
                            </div>
                            <Badge variant="outline" className="mt-0.5 text-[9px] px-1 py-0 capitalize">
                              {mov.origem === "manual" ? "Manual" : "OS"}
                            </Badge>
                          </td>
                          <td className="px-5 py-3">
                            <p className="font-medium">{mov.produtoNome}</p>
                            <p className="font-mono text-xs text-muted-foreground">{mov.produtoSku}</p>
                          </td>
                          <td className="px-5 py-3 text-center font-mono font-semibold">
                            {isEntrada ? "+" : isSaida ? "-" : ""}{mov.quantidade ?? "—"}
                          </td>
                          <td className="px-5 py-3 text-center font-mono text-muted-foreground">{mov.estoqueAnterior}</td>
                          <td className="px-5 py-3 text-center font-mono font-semibold">
                            <span className={mov.estoquePosterior > mov.estoqueAnterior ? "text-emerald-600" : "text-red-500"}>
                              {mov.estoquePosterior}
                            </span>
                          </td>
                          <td className="px-5 py-3 text-xs text-muted-foreground">{mov.motivo ?? "—"}</td>
                        </tr>
                      );
                    })}
                  </tbody>
