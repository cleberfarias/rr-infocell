...
const STATUS_NEXT: Partial<Record<OrdemServicoStatus, OrdemServicoStatus[]>> = {
  ...
  em_analise: ["aguardando_aprovacao", "aguardando_peca", "em_manutencao", "cancelado"],
...
const Ordens = () => {
  ...
  const [viewMode, setViewMode] = useState<"tabela" | "kanban">(() =>
    (localStorage.getItem("rr-ordens-view") as "tabela" | "kanban") ?? "tabela"
  );
  const [density, setDensity] = useState<"default" | "compact">(() =>
    (localStorage.getItem("rr-ordens-density") as "default" | "compact") ?? "default"
  );
  ...
  const mudarStatusMutation = useMutation({
    mutationFn: ({ ordem, novoStatus }: { ordem: OrdemServico; novoStatus: OrdemServicoStatus }) =>
    ...
    onSuccess: (_, { novoStatus }) => {
      ...
      toast.success(`Status atualizado para "${STATUS_KANBAN_LABELS[novoStatus]}".`);
    },
    ...
    return (ordensQuery.data ?? [])
      ...
      .filter((ordem) => {
        ...
        if (atrasoFilter === "atrasadas") {
          ...
          if (!dataPrazo || dataPrazo >= today || ["entregue", "cancelado"].includes(ordem.status)) return false;
        }
  ...
  const tecnicos = useMemo(
    () => Array.from(new Set((ordensQuery.data ?? []).map((o) => o.tecnicoResponsavel ?? "nao_atribuido"))),
    [ordensQuery.data],
  );
    ...
    return (
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="ghost" size="icon" className="h-7 w-7" title="Mudar status" disabled={mudarStatusMutation.isPending}>
        ...
        <DropdownMenuContent align="end">
          ...
          <DropdownMenuSeparator />
          {proximos.map((s) => (
            <DropdownMenuItem key={s} onClick={() => mudarStatusMutation.mutate({ ordem, novoStatus: s })}>
  ...
  const columns: DataTableColumn<OrdemRow>[] = [
    {
      key: "numero",
      header: "OS",
      cell: (o) => <span className="font-mono text-xs text-primary">OS-{o.numero}</span>,
    },
    {
      key: "cliente",
      header: "Cliente",
      cell: (o) => (
        <div>
          ...
          <div className="text-xs text-muted-foreground">{o.cliente?.telefone ?? o.clienteId}</div>
        </div>
      ),
    },
    {
      ...
      cell: (o) => (
        <div>
          <div className="font-medium">{o.aparelho ? `${o.aparelho.marca} ${o.aparelho.modelo}` : "—"}</div>
          <div className="text-xs text-muted-foreground">{o.aparelho?.imeiSerial ? `IMEI ${o.aparelho.imeiSerial}` : o.aparelhoId}</div>
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      cell: (o) => (
        <span className={cn("rounded-md px-2 py-1 text-xs font-medium",
          o.prioridade === "urgente" ? "bg-destructive/10 text-destructive"
          : o.prioridade === "baixa" ? "bg-secondary text-muted-foreground"
    ...
    },
    {
      ...
      className: "text-xs",
      cell: (o) => {
        const { label, classe } = formatPrazo(o.prazoPrometidoEm ?? o.previsaoEntregaEm, o.status);
    ...
    },
    {
      ...
      cell: (o) => (
        <div className="flex justify-end gap-1">
          ...
          <Button variant="ghost" size="icon" asChild title="Detalhes" className="h-7 w-7">
            <Link to={ROUTES.ordemDetalhe(o.id)}><Eye className="h-4 w-4" /></Link>
          </Button>
          <Button variant="ghost" size="icon" asChild title="Checklist" className="h-7 w-7">
            <Link to={ROUTES.checklistOS(o.id)}><ClipboardCheck className="h-4 w-4" /></Link>
          </Button>
          <Button variant="ghost" size="icon" asChild title="Manutenção" className="h-7 w-7">
            <Link to={ROUTES.manutencaoOS(o.id)}><Wrench className="h-4 w-4" /></Link>
          </Button>
          {o.cliente?.telefone && (
  ...
  const isLoading = ordensQuery.isLoading || clientesQuery.isLoading || aparelhosQuery.isLoading;
  const isError = ordensQuery.isError || clientesQuery.isError || aparelhosQuery.isError;
    ...
    <div className="space-y-5">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="hidden md:flex items-center rounded-md border border-border bg-secondary/40 p-0.5">
              ...
              <button
                ...
                className={cn("rounded px-2 py-1 text-xs transition-colors", density === "compact" ? "bg-card text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground")}
              >
                Compacto
              </button>
            </div>
            ...
            <div className="flex items-center rounded-md border border-border bg-secondary/40 p-0.5">
              <button
                ...
                className={cn("flex items-center gap-1.5 rounded px-2.5 py-1.5 text-xs transition-colors", viewMode === "tabela" ? "bg-card text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground")}
              >
              ...
              <button
                ...
                className={cn("flex items-center gap-1.5 rounded px-2.5 py-1.5 text-xs transition-colors", viewMode === "kanban" ? "bg-card text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground")}
              >
                <Kanban className="h-3.5 w-3.5" /> Kanban
              </button>
            </div>
            <Button asChild className="bg-gradient-primary text-primary-foreground shadow-glow">
              <Link to={ROUTES.novaOS}><Plus className="h-4 w-4" /> Nova OS</Link>
            </Button>
          </div>
        }
      />
      ...
      <div className="surface-panel flex flex-wrap items-end gap-3 rounded-lg border border-border p-3">
        <FormField id="ordens-search" label="Buscar ordem" className="min-w-[240px] flex-1">
          <div className="relative">
            ...
            <Input id="ordens-search" className="pl-9" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar por OS, cliente, IMEI ou aparelho..." />
          </div>
        </FormField>
        <FormField id="ordens-status" label="Status" className="min-w-[220px]">
          <Select value={statusFilter} onValueChange={(v) => setStatusFilter(v as OrdemServicoStatus | "todos")}>
            <SelectTrigger id="ordens-status"><SelectValue /></SelectTrigger>
            <SelectContent>
              {OS_STATUS_OPTIONS.map((o) => <SelectItem key={o.value} value={o.value}>{o.label}</SelectItem>)}
            </SelectContent>
          </Select>
        </FormField>
        <FormField id="ordens-prioridade" label="Prioridade" className="min-w-[160px]">
          <Select value={prioridadeFilter} onValueChange={(v) => setPrioridadeFilter(v as typeof prioridadeFilter)}>
            <SelectTrigger id="ordens-prioridade"><SelectValue /></SelectTrigger>
            <SelectContent>
        ...
        <FormField id="ordens-tecnico" label="Técnico" className="min-w-[160px]">
          <Select value={tecnicoFilter} onValueChange={setTecnicoFilter}>
            <SelectTrigger id="ordens-tecnico"><SelectValue /></SelectTrigger>
            <SelectContent>
              <SelectItem value="todos">Todos</SelectItem>
              {tecnicos.map((t) => <SelectItem key={t} value={t}>{t === "nao_atribuido" ? "Não atribuído" : t}</SelectItem>)}
            </SelectContent>
          </Select>
        </FormField>
        <FormField id="ordens-atraso" label="Atraso" className="min-w-[140px]">
          <Select value={atrasoFilter} onValueChange={(v) => setAtrasoFilter(v as typeof atrasoFilter)}>
            <SelectTrigger id="ordens-atraso"><SelectValue /></SelectTrigger>
            <SelectContent>
              <SelectItem value="todos">Todas</SelectItem>
      ...
      {isLoading ? (
        <div className="space-y-5">
          <Card className="surface-panel p-4">
            <div className="flex gap-3">
              <Skeleton className="h-9 flex-1" /><Skeleton className="h-9 w-36" /><Skeleton className="h-9 w-36" />
            </div>
          </Card>
        ...
        <Card className="surface-panel">
          <EmptyState icon={ClipboardList} title="Não foi possível carregar ordens"
            description="Verifique se o backend está rodando."
            actions={<Button variant="outline" onClick={() => { ordensQuery.refetch(); clientesQuery.refetch(); aparelhosQuery.refetch(); }}>Tentar novamente</Button>} />
        </Card>
      ) : viewMode === "tabela" ? (
        <DataTable
          className={cn("surface-panel rounded-lg border border-border", rowClass)}
          columns={columns}
          data={ordens}
          getRowKey={(o) => o.id}
          emptyState={
            <EmptyState icon={ClipboardList} title="Nenhuma ordem encontrada"
              description="Ajuste a busca ou crie uma nova ordem de serviço."
              actions={<Button asChild><Link to={ROUTES.novaOS}>Nova OS</Link></Button>} />
          }
          footer={
            <>
              <span>Mostrando {ordens.length} de {ordensQuery.data?.length ?? 0} ordens</span>
        ...
        <div className="overflow-x-auto pb-4">
          <div className="flex gap-4" style={{ minWidth: `${STATUS_KANBAN.length * 280}px` }}>
            {STATUS_KANBAN.map((status) => {
              const colOrdens = ordens.filter((o) => o.status === status);
              return (
                <div key={status} className="flex w-[268px] shrink-0 flex-col gap-2">
                  {/* Cabeçalho da coluna */}
                  <div className={cn("surface-panel rounded-lg border-t-2 px-3 py-2", STATUS_KANBAN_COLORS[status])}>
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-semibold uppercase tracking-wide">{STATUS_KANBAN_LABELS[status]}</span>
                  ...
                  <div className="space-y-2">
                    {colOrdens.length === 0 ? (
                      ...
                      colOrdens.map((ordem) => {
                        const prazoInfo = formatPrazo(ordem.prazoPrometidoEm ?? ordem.previsaoEntregaEm, ordem.status);
                        const proximos = STATUS_NEXT[status] ?? [];
                        return (
                          <Card key={ordem.id} className="surface-panel overflow-hidden p-0 hover:shadow-lg transition-shadow">
                            <div className="p-3 space-y-2">
                              <div className="flex items-start justify-between gap-2">
                                <div>
                                  <Link to={ROUTES.ordemDetalhe(ordem.id)} className="font-mono text-xs font-bold text-primary hover:underline">
                                    OS-{ordem.numero}
                                  </Link>
                                  <p className="text-sm font-medium leading-tight mt-0.5">{ordem.cliente?.nome ?? "—"}</p>
                                  <p className="text-xs text-muted-foreground">{ordem.aparelho ? `${ordem.aparelho.marca} ${ordem.aparelho.modelo}` : "—"}</p>
                                </div>
                                <span className={cn("shrink-0 rounded-md px-1.5 py-0.5 text-[10px] font-semibold uppercase",
                                  ordem.prioridade === "urgente" ? "bg-destructive/10 text-destructive"
                                  : ordem.prioridade === "baixa" ? "bg-secondary text-muted-foreground"
                                  : "bg-primary/10 text-primary")}>
                                  {ordem.prioridade}
                                </span>
                              </div>

                              <p className="text-xs text-muted-foreground line-clamp-2">{ordem.defeitoRelatado}</p>

                              <div className="flex items-center justify-between gap-2 pt-1 border-t border-border/40">
                                <span className={cn("font-mono text-[10px]", prazoInfo.classe)}>{prazoInfo.label}</span>
                                <div className="flex items-center gap-0.5">
                                  {proximos.length > 0 && (
                                    <DropdownMenu>
                                      <DropdownMenuTrigger asChild>
                                        <Button variant="ghost" size="icon" className="h-6 w-6" disabled={mudarStatusMutation.isPending}>
                                  ...
                                  <Button variant="ghost" size="icon" className="h-6 w-6" asChild title="Detalhes">
                                    <Link to={ROUTES.ordemDetalhe(ordem.id)}><Eye className="h-3 w-3" /></Link>
                                  </Button>
                                  <Button variant="ghost" size="icon" className="h-6 w-6" asChild title="Manutenção">
                                    <Link to={ROUTES.manutencaoOS(ordem.id)}><Wrench className="h-3 w-3" /></Link>
                                  </Button>
                                </div>
                              </div>
                            </div>
