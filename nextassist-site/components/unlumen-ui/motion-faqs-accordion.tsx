...
function AccordionItem({
  ...
  return (
    <motion.div
      layout
      className={cn(
        "faq-motion-item overflow-hidden rounded-[30px] bg-surface text-foreground shadow-xs",
        isOpen && " ",
      )}
    ...
    >
      <button
        ...
        onClick={onToggle}
        className="faq-motion-trigger flex w-full cursor-pointer select-none items-center justify-between gap-4 px-7 py-5 text-left"
      >
        ...
        <motion.span
          ...
          className="faq-motion-icon inline-flex size-12 shrink-0 items-center justify-center text-foreground"
        >
          {isOpen ? (
            <svg
              width="14"
      ...
      >
        <motion.div
          ...
          className="px-7 pb-7"
        >
          <p className="faq-motion-answer text-lg leading-8 font-base tracking-normal text-foreground/75">
            {item.answer}
          </p>
        </motion.div>
      </motion.div>
