...
const key = () => {
  if (!env.INTEGRATIONS_ENCRYPTION_KEY) {
    throw new AppError("encryption_not_configured", "Configure INTEGRATIONS_ENCRYPTION_KEY no backend antes de salvar credenciais.", httpStatus.internalServerError);
  }
...
export function decryptSecrets(payload: { iv: string; tag: string; data: string }) {
  ...
  return JSON.parse(Buffer.concat([decipher.update(Buffer.from(payload.data, "base64")), decipher.final()]).toString("utf8")) as Record<string, string>;
}
