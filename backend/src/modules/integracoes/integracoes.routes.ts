...
async function saveSecrets(
  ...
  const existingEncrypted = existingSnap.data()?.encrypted as
    | { iv: string; tag: string; data: string }
    | undefined;
  const existing = existingEncrypted ? decryptSecrets(existingEncrypted) : {};
  const merged = { ...existing, ...incoming };
  await ref.set(
    { encrypted: encryptSecrets(merged), updatedAt: new Date().toISOString() },
    { merge: true },
  );
...
integracoesRoutes.get(
  ...
  asyncHandler(async (request, response) => {
    ...
    response
      ...
      .json({ data: snap.exists ? snap.data() : { fiscal: null, payment: null } });
  }),
);
...
integracoesRoutes.post(
  ...
  asyncHandler(async (request, response) => {
    ...
    response
      .status(httpStatus.ok)
      .json({ data: { authorizationUrl: await createAuthorizationUrl(tenantId) } });
  }),
);

integracoesRoutes.get(
  "/mercado-pago/terminals",
  asyncHandler(async (request, response) => {
    const tenantId = getRequestTenantId(request as TenantRequest);
...
integracoesRoutes.post(
  ...
  asyncHandler(async (request, response) => {
    ...
    if (!parsed.success)
      throw new AppError(
        "validation_error",
        parsed.error.errors[0]?.message ?? "Cobranca invalida.",
        httpStatus.badRequest,
      );
    const tenantId = getRequestTenantId(request as TenantRequest);
    response
      .status(httpStatus.created)
      .json({ data: await createPaymentTransaction(tenantId, parsed.data) });
  }),
);

integracoesRoutes.get(
  "/mercado-pago/orders/:id",
  asyncHandler(async (request, response) => {
    const tenantId = getRequestTenantId(request as TenantRequest);
    response
      .status(httpStatus.ok)
      .json({ data: await syncPaymentTransaction(tenantId, String(request.params.id)) });
  }),
);

integracoesRoutes.post(
  "/mercado-pago/orders/:id/cancel",
  asyncHandler(async (request, response) => {
    const tenantId = getRequestTenantId(request as TenantRequest);
    response
      .status(httpStatus.ok)
      .json({ data: await cancelPaymentTransaction(tenantId, String(request.params.id)) });
  }),
);

integracoesRoutes.put(
  "/fiscal",
  asyncHandler(async (request, response) => {
    ...
    if (!parsed.success)
      throw new AppError(
        "validation_error",
        parsed.error.errors[0]?.message ?? "Configuracao fiscal invalida.",
        httpStatus.badRequest,
      );
    const tenantId = getRequestTenantId(request as TenantRequest);
...
integracoesRoutes.put(
  ...
  asyncHandler(async (request, response) => {
    ...
    if (!parsed.success)
      throw new AppError(
        "validation_error",
        parsed.error.errors[0]?.message ?? "Configuracao de pagamento invalida.",
        httpStatus.badRequest,
      );
    const tenantId = getRequestTenantId(request as TenantRequest);
    const input = parsed.data as Record<string, unknown>;
    ...
    const configured = Object.keys(secrets).length
      ? await saveSecrets(tenantId, "payment", secrets)
      : (currentPayment.secretsConfigured ?? []);
    const payment = {
      ...currentPayment,
      ...input,
...
integracoesRoutes.post(
  ...
  asyncHandler(async (request, response) => {
    ...
    if (kind !== "fiscal" && kind !== "payment")
      throw new AppError("invalid_integration", "Integracao invalida.", httpStatus.badRequest);
    const tenantId = getRequestTenantId(request as TenantRequest);
    const snap = await requireDb().collection("tenantIntegrations").doc(tenantId).get();
    ...
    if (!settings)
      throw new AppError(
        ...
        "Salve a configuracao antes de testar.",
        httpStatus.badRequest,
      );
    if (kind === "payment" && settings.provider === "mercado_pago" && settings.oauthConnected)
      return response.status(httpStatus.ok).json({ data: await testMercadoPago(tenantId) });
    response.status(httpStatus.ok).json({
      data: {
        ok: false,
        status: "pending_provider_adapter",
        message:
          "Configuracao validada. O teste externo sera habilitado com o adaptador do provedor.",
      },
    });
  }),
);
